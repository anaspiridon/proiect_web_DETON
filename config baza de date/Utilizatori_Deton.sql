...
--for i in 1..100
--loop
--select nume, prenume, trim(lower(prenume))||'.'||trim(lower(nume)) BULK COLLECT INTO v_nume, v_prenume, v_utilizator from vizitatori;
--
--SELECT parola into v_parola FROM
--( SELECT parola FROM parole
--ORDER BY dbms_random.value )
--WHERE rownum = 1;
--
--insert into utilizatoriDeton(ID, nume, prenume, username, password, tip, id_institutie) VALUES(seq_vizitatoriDeton.nextval, v_nume(i), v_prenume(i),v_utilizator(i), v_parola, 'vizitator', 0);
--END LOOP;
--END;
--
--/
...
type nume IS VARRAY(1000) OF VARCHAR2(255);
type prenume IS VARRAY(1000) OF VARCHAR2(255);
type id_institutie IS VARRAY(1000) OF NUMBER;
v_utilizator  username;
v_nume nume;
v_prenume prenume;
v_parola varchar2(30);
v_count integer;
v_id_institutie id_institutie;
BEGIN
FOR i IN 1..1000
LOOP
SELECT distinct (trim(lower(prenume))||'.'||trim(lower(nume))),nume, prenume, id_institutie BULK COLLECT INTO v_utilizator, v_nume, v_prenume, v_id_institutie  from detinuti;
SELECT parola into v_parola FROM
( SELECT parola FROM parole
ORDER BY dbms_random.value )
WHERE rownum = 1;

insert into utilizatoriDeton(ID, nume, prenume, username, password, tip, id_institutie) VALUES(seq_detinutiDeton.nextval, v_nume(i), v_prenume(i),v_utilizator(i), v_parola, 'detinut', v_id_institutie(i));
END LOOP;
END;
