...
function get_imput($nume = '', $prenume = '', $nrDosar = '', $sPedeapsa = '',$data_nastere='')
{
    echo <<<END
  ...
  <p><label for="pedeapsa">Pedeapsa:</label>
        <select name="pedeapsa" id="pedeapa">
...
END;
   ...
   $sql="select ID_INSTITUTIE, NUME_INSTITUTIE from institutie";
   $stmt=oci_parse($conn, $sql);
   oci_execute($stmt);
    echo "<p><label for =\"tip\" > Tip institutie: </label>
        <select name=\"gen\" id=\"gen\">";
   while (($row = oci_fetch_assoc($stmt)) != false) {
         echo " <option value = ". $row['ID_INSTITUTIE'].'>'.$row['NUME_INSTITUTIE']."</option>";}
  echo <<<END
  </select>  
...
if(empty($_REQUEST['nume']))
{
  ...
  else
  {
        ...
        $idInstitutie = $_REQUEST['idinsitutie'];
